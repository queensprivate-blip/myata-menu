...
const BAR_CATEGORY_GROUPS = [
  { title:'Напитки и кофе', ids:['napitki','kofe'] },
  { title:'Чай', ids:['chaynaya-karta','kitayskie-chai','dobavki-k-chayu','nechaynyy-chay'] },
  { title:'Пиво и сидр', ids:['pivo-i-sidr'] },
  { title:'Вино', ids:['beloe-vino','krasnoe-vino','igristye-vina'] },
  { title:'Крепкий алкоголь', ids:['viski','vodka','nastoyki','dzhin','rom','konyak','tekila','bitter','vermut'] },
  { title:'Коктейли', ids:['kokteyli'] },
  { title:'Закуски, пицца и десерты', ids:['zakuski','pitstsa','deserty'] },
  { title:'Дополнительные услуги', ids:['dopolnitelnye-uslugi'] },
];
...
function bind(render){
 ...
 document.querySelectorAll('[data-item-save]').forEach(b=>b.onclick=async()=>{
  ...
  try{
    ...
    window.dispatchEvent(new CustomEvent('myata-menu-updated'));
...
export async function renderAdvancedAdmin(app,{session,backgroundUrl,logoUrl,venueName='МЯТА PLATINUM'}){
