...
import logoUrl from './myata-platinum-logo.png';
import { brand, applyBrandTheme } from './brand-config.js';
import { barCategories, importedPromotions, hookahItems } from './menu-data.js';
...
const groupedBarCategories = [
  {
    id: 'soft-drinks',
    title: 'Напитки и кофе',
    note: 'Безалкогольные напитки, лимонады и кофе',
    sections: [categoryById['napitki'], categoryById['kofe']].filter(Boolean),
  },
  {
    id: 'tea',
    title: 'Чай',
    note: 'Классические, китайские и фруктовые чаи',
    sections: [categoryById['chaynaya-karta'], categoryById['kitayskie-chai'], categoryById['dobavki-k-chayu'], categoryById['nechaynyy-chay']].filter(Boolean),
  },
  {
    id: 'beer-cider',
    title: 'Пиво и сидр',
    note: 'Пивная карта заведения',
    sections: [categoryById['pivo-i-sidr']].filter(Boolean),
  },
  {
    id: 'wine',
    title: 'Вино',
    note: 'Белое, красное и игристое',
    sections: [categoryById['beloe-vino'], categoryById['krasnoe-vino'], categoryById['igristye-vina']].filter(Boolean),
  },
  {
    id: 'strong-alcohol',
    title: 'Крепкий алкоголь',
    note: 'Виски, водка, настойки, джин, ром и другое',
    sections: [categoryById['viski'], categoryById['vodka'], categoryById['nastoyki'], categoryById['dzhin'], categoryById['rom'], categoryById['konyak'], categoryById['tekila'], categoryById['bitter'], categoryById['vermut']].filter(Boolean),
  },
  {
    id: 'cocktails',
    title: 'Коктейли',
    note: 'Классические и фирменные коктейли',
    sections: [categoryById['kokteyli']].filter(Boolean),
  },
  {
    id: 'food',
    title: 'Закуски, пицца и десерты',
    note: 'Еда и десерты к вашему отдыху',
    sections: [categoryById['zakuski'], categoryById['pitstsa'], categoryById['deserty']].filter(Boolean),
  },
  {
    id: 'services',
    title: 'Дополнительные услуги',
    note: 'Аренда приставки и другие услуги',
    sections: [categoryById['dopolnitelnye-uslugi']].filter(Boolean),
  },
];
...
const menuData = {
  bar: {
    title: 'Бар',
    subtitle: 'Барное меню, кухня и дополнительные услуги',
...
const money = (value) => `${new Intl.NumberFormat(brand.locale).format(value)} ${brand.currency}`;

const CHOICE_STORAGE_KEY = `${brand.storagePrefix}-choice-v1`;
const ADMIN_STORAGE_KEY = `${brand.storagePrefix}-admin-overrides-v1`;
      ...
      <dialog class="venue-dialog" id="venue-dialog" aria-labelledby="venue-dialog-title">
        <div class="venue-dialog-card">
          ...
          <div class="venue-dialog-heading">
            ...
            <h2 id="venue-dialog-title">${esc(brand.venueName)}</h2>
          </div>
...
function adminPage() {
  return renderAdvancedAdmin(app, { session: adminSession, backgroundUrl, logoUrl, venueName: brand.venueName });
}
